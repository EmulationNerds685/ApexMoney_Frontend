  ...
  return (
    <>
      <header className="relative my-4 bg-white/70 backdrop-blur-md shadow-sm rounded-4xl w-full max-w-7xl mx-auto mt-4 px-4 sm:px-6 py-3 flex justify-between items-center z-20">
        ...
        {/* Navbar Links */}
        <nav className="hidden md:flex items-center gap-8 text-gray-700 font-medium bg-purple-100 p-3 rounded-4xl">
          <Link to="/" className="text-purple-600 hover:text-purple-800 transition">
            Home
          </Link>
          ...
          {/* Dashboard message only when signed in */}
          {user && (
            <Link
              to="/dashboard"
              className=" absolute text-lg right-24 font-semibold text-purple-700 hover:text-purple-900 transition"
            >
              Dashboard 👉
            </Link>
          )}
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-md w-full max-w-7xl mx-auto rounded-xl shadow-lg z-10 mt-2">
          <nav className="flex flex-col items-center gap-6 text-gray-700 font-medium py-6">
            ...
            {/* Dashboard message in mobile menu */}
            <div className="space-x-4">
            {user && (
              <Link
                to="/dashboard"
                className="text-sm font-semibold text-red-700 hover:text-red-900 transition"
              >
                Dashboard 👉
              </Link>
            )}
