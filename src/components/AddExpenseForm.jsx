...
export function AddExpenseForm({ expenseToEdit, onFormSubmit, onCancelEdit }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Success - response.data is always truthy for 2xx responses
      if (onFormSubmit) {
        onFormSubmit();
      }
      if (!formData._id) {
      ...
      if (validationErrors) {
        ...
        const messages = validationErrors.map(e => e.field && e.message ? `${e.field}: ${e.message}` : e.message || String(e)).join('\n');
