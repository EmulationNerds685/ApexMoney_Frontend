  ...
  return (
    <motion.div className="bg-slate-50 p-4 sm:p-6 rounded-2xl" initial="hidden" animate="visible" variants={containerVariants}>
      <motion.div variants={itemVariants} className="flex flex-wrap justify-between items-center gap-4 mb-6">
        ...
        <Link to="/income">
          <motion.button className="flex items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-semibold shadow-lg" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
        ...
        <motion.div variants={itemVariants} className="space-y-3 mb-6">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-10 py-3 border border-gray-200 rounded-xl bg-white focus:ring-2 focus:ring-emerald-500 focus:border-emerald-300 transition text-sm shadow-sm"
            />
            {searchQuery && (
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-1.5 text-xs text-gray-500 font-medium"><Filter size={14} />Filters:</div>

            <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="px-3 py-1.5 text-xs border border-gray-200 rounded-lg bg-white focus:ring-2 focus:ring-emerald-500 transition">
              <option value="">All Sources</option>
              {allCategories.map(cat => <option key={cat} value={cat}>{cat}</option>)}
            </select>

            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="px-3 py-1.5 text-xs border border-gray-200 rounded-lg bg-white focus:ring-2 focus:ring-emerald-500 transition">
            ...
            {allTags.map(tag => (
              <button
                ...
                className={`px-2.5 py-1 text-xs rounded-full font-medium transition ${selectedTags.includes(tag) ? 'bg-emerald-600 text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-emerald-50 hover:text-emerald-600'
                  }`}
              >#{tag}</button>
            ))}

            {hasFilters && (
              <button onClick={() => { setSearchQuery(''); setSelectedTags([]); setSelectedCategory(''); }} className="px-2.5 py-1 text-xs text-red-500 hover:text-red-700 font-medium">
                Clear All
              </button>
            )}
          </div>
          ...
          {hasFilters && (
            <p className="text-xs text-gray-500">
              ...
              {filteredIncomes.length !== incomes.length && <> • Filtered total: <strong className="text-green-600">{getSymbol(userCurrency)}{filteredTotal.toLocaleString(undefined, { maximumFractionDigits: 2 })}</strong></>}
            </p>
          )}
        </motion.div>
      )}
      ...
      {incomes.length === 0 ? (
        ...
        <motion.div variants={itemVariants} className="text-center py-12 bg-white rounded-2xl shadow-md">
          ...
          <h3 className="text-lg font-semibold text-gray-600">No matching income</h3>
        ...
        <>
          <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8" variants={containerVariants}>
            <AnimatePresence>
              {filteredIncomes.map((income) => (
                <motion.div key={income._id} className="bg-white rounded-2xl shadow-lg p-5 flex flex-col justify-between transition-shadow hover:shadow-xl" variants={itemVariants} layout exit={{ opacity: 0, scale: 0.8 }}>
                  <div>
                    <div className="flex justify-between items-start">
                      <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-semibold">{income.source || income.category || 'Uncategorized'}</span>
                    ...
                    <div className="flex items-center gap-2 my-4">
                      <p className="text-3xl font-bold text-gray-800">{getSymbol(income.currency)}{income.amount.toLocaleString()}</p>
                    </div>
                    <p className="text-gray-500 text-sm break-words mb-3 min-h-[40px]">{income.notes || 'No notes provided.'}</p>

                    {income.tags?.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {income.tags.map((tag, i) => (
                          <span key={i} className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-full text-[11px] font-medium">
                            <Tag size={10} />{tag}
                          </span>
                        ))}
                      </div>
          ...
          <motion.div variants={itemVariants} className="bg-white p-6 rounded-2xl shadow-lg">
            <div className="flex items-center space-x-4">
              <div className="p-3 rounded-full bg-green-100"><TrendingUp className="w-7 h-7 text-green-600" /></div>
              <div>
                <p className="text-lg font-medium text-gray-500">Total Income</p>
                <h3 className="text-3xl font-bold text-green-600">{getSymbol(userCurrency)}{totalIncome.toLocaleString(undefined, { maximumFractionDigits: 2 })}</h3>
              </div>
            </div>
          </motion.div>
        </>
